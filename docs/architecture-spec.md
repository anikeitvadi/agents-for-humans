...
- **Sample case, not real uploads:** nobody should upload real immigration documents to a hackathon URL. The "load sample case" button (`GET /api/sample-case`) runs real extraction (`agent/llm/extract.py::extract_fields`) against three synthetic specimen-format images in `fixtures/sample_case/specimens/` (I-94, I-797, passport — invented data, real-format layout), via a client that is live Bedrock when AWS credentials are configured and a recorded replay of the same specimen images otherwise (`agent/llm/document_client.py`). Every response is labeled `mode: "live" | "recorded"` so the UI never presents a recorded replay as a live parse. This is the *only* live-demo path and doubles as the extraction test fixture.
